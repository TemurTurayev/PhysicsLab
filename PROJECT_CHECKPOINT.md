...
**Дата:** 2025-11-29
**Версия:** 0.3.0 (Mission 5.1.1 Complete)
**Статус:** В разработке - День 1 из 3 (Модуль Алгебры) - Ready for Testing
...
### День 2 - Interactive Features
- [ ] Add interactive sliders for function parameters
- [ ] Implement full theory panel with markdown + KaTeX
- [ ] Create Mission 5.1.2 (Linear functions family)
- [ ] Add slider-based function transformation
- [ ] Polish UI/UX

### День 3 - Polish & Expand
- [ ] Create Mission 5.1.3 (Quadratic functions)
- [ ] Add parabola visualization
- [ ] Implement mission switching system
...
## 🧪 Как протестировать Mission 5.1.1

**Миссия готова к тестированию!** Вот как проверить, что всё работает:
...
### 2. Проверить UI
- ✅ Заголовок: "Миссия 5-1-1: Что такое функция?"
- ✅ Кнопка "📖 Теория" в Header
- ✅ MathCanvas показывает placeholder: "📈 Запусти код с mathlab..."
- ✅ CodeEditor показывает starter code с `Function("___")`
- ✅ Кнопки "▶ Запустить" и "🔄 Сброс"

### 3. Открыть теорию
Нажми "📖 Теория" → должна открыться боковая панель с:
- Брифинг (ситуация, задача, контекст)
- Теоретическая справка с формулами KaTeX: $f(x) = 2x$
- Кнопка "Показать подсказку" (3 уровня подсказок)

### 4. Запустить неправильный код
Оставь `Function("___")` и нажми "▶ Запустить":
...
```
Нажми "▶ Запустить":
- ✅ В консоли: значения f(0), f(1), f(5), f(-3)
- ✅ MathCanvas показывает **линейный график** y = 2x
- ✅ Синяя прямая через начало координат
- ✅ Координатные оси, сетка, легенда "f(x) = 2x"

### 6. Проверить другие функции
Попробуй изменить:
- `Function("x**2")` → парабола
- `Function("sin(x)")` → синусоида
- `Function("sqrt(abs(x))")` → корень

### 7. Проверить сброс
Нажми "🔄 Сброс":
- ✅ Код вернулся к starter code
- ✅ Canvas очистился
- ✅ Консоль очистилась
